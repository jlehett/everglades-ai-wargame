from agents.Distributional_RL.QNetwork import QNetwork
from agents.Distributional_RL.Multi_Step import NStepModule
import torch
...
N_ATOM = 51 # Number of items in the distributions
V_MIN = -5. # Minimum reward
V_MAX = 10. # Maximum reward
V_RANGE = np.linspace(V_MIN, V_MAX, N_ATOM) # N_ATOM evenly-spaced values across the range of rewards
...
class DQNAgent():
    ...
    def optimize_model(self):
        ...
        #print(batch.next_state_swarms[0])
        ...
        swarm_action_batch = torch.from_numpy(np.asarray(batch.swarm_action)).unsqueeze(1).type(torch.LongTensor)
        ...
        non_final_next_state_swarms_batch = nth_next_state_swarms_batch[non_final_mask, :, :, :] # (swarm_action_batch, NUM_GROUPS, INPUT_SIZE, N_ATOM)
        #print(non_final_next_state_swarms_batch.size())

        # Compute the swarm's predicted qs for the current state
        #print(self.policy_net(swarm_state_batch).size())
        state_swarms_predicted_q_batch = self.policy_net(swarm_state_batch).gather(1, swarm_action_batch)
        state_swarms_predicted_q_batch = state_swarms_predicted_q_batch.expand(-1, N_ATOM) # (swarm_action_batch, N_ATOM)
        #print(state_swarms_predicted_q_batch.size())

        # Compute the swarm's future value for next states
        next_state_swarms_predicted_qs_batch = torch.zeros((BATCH_SIZE, NUM_GROUPS, self.num_nodes, N_ATOM), device=device)
        #print(next_state_swarms_predicted_qs_batch.size())
        for swarm_num in range(NUM_GROUPS):
            print(next_state_swarms_predicted_qs_batch[non_final_mask, swarm_num, :, :].size())
            # (non_final_mask, OUTPUT_SIZE, N_ATOM)
        ...
        # DRL: Find the action distribution with the best MEAN
